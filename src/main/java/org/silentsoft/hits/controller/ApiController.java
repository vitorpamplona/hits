        ...
        if (uri.equals(urn)) {
            if (urn.length() == 0) {
                ...
                if (urnsEntity == null) {
                    ...
                    List<HitsUrnsStatisticsEntity> entities = hitsUrnsStatisticsRepository.findById_UrnIdOrderById_DateDesc(urnsEntity.getUrnId());
                    if (entities.size() == 0) {
                        return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
                    } else {
                        Map<String, Object> result = new HashMap<>();
                        result.put("total", entities.stream().mapToLong(HitsUrnsStatisticsEntity::getCount).sum());
                        result.put("monthly", entities.stream().filter(entity -> entity.getId().getDate().toLocalDate().isAfter(LocalDate.now().minusMonths(1))).mapToLong(HitsUrnsStatisticsEntity::getCount).sum());
                        result.put("weekly", entities.stream().filter(entity -> entity.getId().getDate().toLocalDate().isAfter(LocalDate.now().minusWeeks(1))).mapToLong(HitsUrnsStatisticsEntity::getCount).sum());
                        ...
                        for (Map.Entry<String, List<HitsUrnsStatisticsEntity>> entry : yearlyMap.entrySet()) {
                            ...
                            String from = entry.getValue().stream().min(Comparator.comparing(value -> value.getId().getDate())).get().getId().getDate().toString();
                            String to = entry.getValue().stream().max(Comparator.comparing(value -> value.getId().getDate())).get().getId().getDate().toString();
